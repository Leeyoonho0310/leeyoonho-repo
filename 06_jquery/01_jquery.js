...
function useAppend() {
  ...
  // red 라는 class 를 가진 모든 요소의 앞에 div.orange 요소 추가됨
  // 반복문 돌면서 지정하지 않아도 모든 요소에 지정됨 -> jquery가 알아서 함
...
function useRemove() {
  // remove() : 선택된 요소 삭제
  // red 라는 id를 가진 요소만 삭제
  $('#red').remove();

  // empty() : 선택된 요소의 모든 content 삭제
  // container 라는 class 를 가진 요소의 content(자식, 텍스트 모두)를 삭제
  $('.container').empty();
  // js는 empty 없음 -> innerHTML 속성을 빈 문자열로 하면 동일하게 가능
...
function useParent() {
  ...
  // next() : 자신의 다음 형제 요소 호출
  // === js nextElementSibling
  console.log('next', $('#red').next());

  // prev() : 자신의 이전 형제 호출
  // === js previousElementSibling
  // container class 를 가진 요소의 두 번째 자식이면서 box class 를 가진 요소를 선택
...
function controlClass() {
  ...
  $("#controlClass").removeClass("text"); // 클래스 제거
  // $("#controlClass").removeClass(); // 모든 클래스 제거

  console.log($("#controlClass").hasClass("red")); // 클래스 포함 여부
